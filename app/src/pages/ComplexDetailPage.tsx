...
import type { Conformation, LeaderboardEntry, Pocket, ProteinViewerHandle, DockingInfo } from '../types';
...
export function ComplexDetailPage() {
  ...
  const handleStartDocking = useCallback((info: DockingInfo) => {
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-[11px] uppercase tracking-wider text-text-primary font-bold">Dock — Pocket #{activeDocking.pocket.pocket_id}</span>
              <span className={`font-mono text-[9px] uppercase px-1.5 py-0.5 rounded border ${activeDocking.sourceType === 'monomer' ? 'text-blue-400 bg-blue-500/10 border-blue-500/20' : 'text-purple-400 bg-purple-500/10 border-purple-500/20'}`}>{activeDocking.sourceType}</span>
            </div>
            <button onClick={() => { handleUndock(); setActiveDocking(null); }} className="font-mono text-[10px] uppercase tracking-wider px-2 py-1 rounded border border-border text-text-muted hover:text-text-primary hover:border-border-subtle transition-colors">✕ Close</button>
          </div>
          <DockingPanel pocket={activeDocking.pocket} proteinPdbId={activeDocking.proteinPdbId} sourceType={activeDocking.sourceType}
            onConformationChange={(confs, mode) => { const target = activeDocking.activeTab === 'monomer' ? 'monomer' : 'complex'; if (confs) { viewerRef.current?.setConformations?.(confs, mode!, target); } else { viewerRef.current?.clearConformations?.(target); } }}
            onUndock={handleUndock} onDockingComplete={handleDockingComplete} />
        </div>
      );
    }
    if (selectedEntry) { return <DockingResultDetail entry={selectedEntry} onConformationSelect={handleConformationSelect} />; }
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-2">
        ...
        <span className="font-mono text-[10px] uppercase tracking-wider text-text-muted">Select a pocket to dock</span>
        <span className="font-mono text-[9px] text-text-muted opacity-60 text-center">Click "Dock Molecule" on any pocket, or select a result from the leaderboard</span>
      </div>
    );
  };
      ...
      <div className="w-full max-w-[1400px] px-6 py-[48px] flex flex-col gap-6">
        ...
        {!loading && !error && complex && (
          <div className="flex flex-col">
            ...
            <ProteinViewer ref={viewerRef} monomerUrl={complex.monomer_structure_url} complexUrl={complex.complex_structure_url} monomerPlddt={complex.monomer_plddt_avg} dimerPlddt={complex.dimer_plddt_avg} disorderDelta={complex.disorder_delta} />
              ...
              <div className="flex-1 min-w-0 bg-bg-secondary border border-border rounded flex flex-col">
                <div className="flex items-center justify-between px-5 py-3 border-b border-border-subtle">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-[11px] uppercase tracking-wider text-text-primary font-bold">Docking Leaderboard</span>
                    {leaderboard.length > 0 && (<span className="bg-accent/20 text-accent text-[9px] px-1.5 py-0.5 rounded-full font-bold">{leaderboard.length}</span>)}
                  </div>
                </div>
                <div className="p-3 max-h-[400px] overflow-y-auto">
                  <DockingLeaderboard entries={leaderboard} activeEntryId={selectedEntry?.id} onSelect={handleLeaderboardSelect} onRemove={handleLeaderboardRemove} />
                </div>
              </div>
              <div className="flex-1 min-w-0 bg-bg-secondary border border-border rounded p-5">{renderRightPanel()}</div>
            </div>
            <BindingSitesPanel complexId={id} onHighlightPocket={(indices, target) => viewerRef.current?.highlightPocket?.(indices, target)}
              onClearHighlight={(target) => { viewerRef.current?.clearPocketHighlight?.(target); viewerRef.current?.clearConformations?.(target); }}
              ...
              onStartDocking={handleStartDocking} activeDockingPocketId={activeDocking?.pocket?.pocket_id} />
          </div>
        )}
      </div>
    </div>
