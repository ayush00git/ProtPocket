...
export function HomePage() {
  ...
  const pipelineSteps = [
    { num: '01', title: 'Discover', desc: 'Sifting through 1.7M AlphaFold complexes for high-confidence, undrugged WHO and human disease targets.' },
    { num: '02', title: 'Reveal', desc: 'Calculating thermodynamic Disorder Delta to find hidden structures that only stabilize upon dimerization.' },
    { num: '03', title: 'Target', desc: 'Running fpocket to autonomously identify geometric cavities situated on these stabilizing interfaces.' },
    { num: '04', title: 'Dock', desc: 'Performing high-throughput molecular docking of candidates directly inside the pocket using AutoDock Vina.' }
  ];
      ...
      <section className="w-full border-b border-border bg-bg-primary">
        <div className="max-w-[800px] mx-auto px-6 py-[100px] flex flex-col gap-8 text-center items-center">
          <div className="flex flex-col gap-5 items-center w-full">
            <span className="font-mono text-[11px] uppercase tracking-wider bg-accent-dim dark:border dark:border-accent/30 text-[#1E3A8A] dark:text-accent px-4 py-1.5 rounded-full transition-colors duration-150 shadow-sm">End-to-End Drug Lead Generation Platform</span>
            <h1 className="font-display font-bold text-[52px] leading-tight text-[#1E3A8A] dark:text-text-primary transition-colors duration-150">From predicted complex to drug lead. Fast.</h1>
            <p className="font-body text-[18px] text-text-secondary leading-relaxed max-w-[680px] transition-colors duration-150">ProtPocket surfaces the highest-priority undrugged targets from the AlphaFold complex dataset. We compute structural gap scores and interface pocket viability to jumpstart novel therapeutic design.</p>
          </div>
      ...
      <section className="w-full border-b border-border bg-bg-primary py-[80px]">
        <div className="max-w-[1200px] mx-auto px-6 flex flex-col gap-12">
          <div className="flex flex-col gap-2 items-center text-center max-w-[700px] mx-auto">
            ...
            <p className="font-body text-[16px] text-text-secondary leading-relaxed">We cross-reference AlphaFold structures, ChEMBL drug data, and WHO priority lists in real-time, feeding the best candidates into our autonomous pocket analysis engine.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {pipelineSteps.map((step, idx) => (
              <div key={idx} className="flex flex-col gap-4 p-6 bg-bg-secondary border border-border rounded-lg relative overflow-hidden group hover:border-accent transition-colors duration-300 shadow-sm">
                <div className="absolute -right-4 -top-6 text-[100px] font-display font-bold text-border-subtle opacity-30 select-none group-hover:text-accent-dim transition-colors duration-300">{step.num}</div>
      ...
      <section className="w-full border-b border-border bg-bg-secondary py-[80px]">
        <div className="max-w-[1400px] mx-auto px-6 flex flex-col xl:flex-row gap-12 items-center">
          <div className="flex-1 xl:max-w-[450px] flex flex-col gap-6">
            <span className="font-mono text-[12px] uppercase text-accent tracking-widest border border-accent/30 bg-accent/10 px-3 py-1 rounded w-fit">The Holy Grail of PPIs</span>
            <h2 className="font-display font-bold text-[32px] text-text-primary leading-tight">Targeting the Disorder Delta</h2>
            <p className="font-body text-[16px] text-text-secondary leading-relaxed">Many proteins appear completely disordered as single monomers, but snap into highly stable structures when bound to their partner. We call this the <strong>Disorder Delta</strong>.</p>
            <p className="font-body text-[16px] text-text-secondary leading-relaxed">By combining thermodynamic confidence scoring (pLDDT) with geometric cavity detection (<code className="font-mono text-[14px] bg-bg-tertiary px-1 rounded">fpocket</code>), ProtPocket flags cavities that sit exactly on these newly stabilized interfaces. These are the prime targets for Protein-Protein Interaction (PPI) inhibitors.</p>
          </div>
          <div className="xl:flex-[2.5] w-full bg-bg-tertiary border border-border rounded-lg flex flex-col md:flex-row items-center justify-center relative overflow-hidden shadow-inner h-[450px]">
             <div className="flex-1 w-full h-full relative border-r border-border">
                <MolstarPanel structureUrl="https://alphafold.ebi.ac.uk/files/AF-Q55DI5-F1-model_v6.cif" label="Monomer (Q55DI5)" visible={true} representation="cartoon" plddt={42.1} hideControls={true} />
             </div>
             <div className="flex-1 w-full h-full relative">
                <MolstarPanel structureUrl="https://alphafold.ebi.ac.uk/files/AF-0000000066503175-model_v1.cif" label="Complex (Homodimer)" visible={true} representation="cartoon" plddt={85.4} hideControls={true} />
             </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-[1200px] mx-auto px-6 py-[80px] flex flex-col gap-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {DEMO_PROTEINS.map((protein) => (
              <div key={protein.id} onClick={() => navigate(`/complex/${protein.id}`)} className="flex flex-col gap-4 p-6 bg-bg-secondary border border-border rounded-lg hover:border-accent hover:shadow-[0_4px_20px_rgba(0,0,0,0.05)] cursor-pointer transition-all duration-300">
                <div className="flex justify-between items-start">
                  <span className="font-display font-bold text-[22px] text-accent tracking-tight">{protein.label}</span>
                  <div className="w-8 h-8 rounded-full bg-bg-tertiary flex items-center justify-center text-text-muted group-hover:text-accent transition-colors">↗</div>
                </div>
