...
import type { Pocket, Conformation } from '../../types';

interface BindingSitesPanelProps {
  complexId: string | undefined;
  onHighlightPocket?: (indices: number[], target: string) => void;
  ...
  onStartDocking?: (info: { pocket: Pocket; sourceType: string; proteinPdbId: string; activeTab?: string }) => void;
 ...
 */
export function BindingSitesPanel({ complexId, onHighlightPocket, onClearHighlight, monomerStructureUrl, complexStructureUrl, onConformationChange, onStartDocking, activeDockingPocketId }: BindingSitesPanelProps) {
  const [enabled, setEnabled] = useState(true);
  const [activePocketIdx, setActivePocketIdx] = useState<string | number | null>(null);
  const [showAll, setShowAll] = useState(false);
  const [activeTab, setActiveTab] = useState<'monomer' | 'complex' | 'comparison'>('monomer');
  ...
  const handleHighlight = (residueIndices: number[], pocketIdx: string | number, targetOverride?: string) => {
  ...
  const handleTabSwitch = (tab: 'monomer' | 'complex' | 'comparison') => {
  ...
  const handleStartDocking = useCallback((info: { pocket: Pocket; sourceType: string; proteinPdbId: string }) => {
