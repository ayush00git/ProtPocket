...
interface PocketTableRowProps {
  pocket: Pocket;
  activeTab: string;
  isActive: boolean;
  onHighlight: (residueIndices: number[]) => void;
  proteinPdbId?: string;
  sourceType?: string;
  onStartDocking?: (info: { pocket: Pocket; sourceType: string; proteinPdbId: string }) => void;
  isDockingActive?: boolean;
  onConformationChange?: (confs: unknown, mode: unknown) => void;
}
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <button
            ...
            onClick={() => {
              ...
              onStartDocking?.({ pocket, sourceType: sourceType || 'dimer', proteinPdbId });
            }}
            className={`font-mono text-xs uppercase tracking-wider px-2 py-1 rounded border transition-colors duration-150 ${
              !proteinPdbId
