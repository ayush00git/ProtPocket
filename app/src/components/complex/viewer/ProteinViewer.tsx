...
import type { Conformation, ProteinViewerHandle, MolstarPanelHandle } from '../../../types';
...
interface ProteinViewerProps {
  ...
  monomerPlddt?: number;
  dimerPlddt?: number;
  disorderDelta?: number;
  monomerConformations?: Conformation[] | null;
...
export const ProteinViewer = forwardRef<ProteinViewerHandle, ProteinViewerProps>(
  ({
    ...
    const [complexDocking, setComplexDocking] = useState<DockingState>({ conformations: null, activeMode: null });
    ...
    const monomerViewerRef = useRef<MolstarPanelHandle>(null);
    const complexViewerRef = useRef<MolstarPanelHandle>(null);

    useImperativeHandle(ref, () => ({
      highlightPocket: (residueIndices: number[], target = 'complex') => {
        setActiveHighlight({ indices: residueIndices, target });
      },
      clearPocketHighlight: () => {
        setActiveHighlight(null);
      },
      setConformations: (confs: Conformation[], mode: number, target = 'complex') => {
