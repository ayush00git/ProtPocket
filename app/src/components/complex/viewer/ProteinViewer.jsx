...
export const ProteinViewer = forwardRef(
  ({ monomerUrl, complexUrl, monomerPlddt, dimerPlddt, disorderDelta, conformations = null, activeMode = null }, ref) => {
    ...
    const [conformationState, setConformationState] = useState({ conformations: null, activeMode: null });
    const monomerViewerRef = useRef(null);
    const complexViewerRef = useRef(null);

    const resolvedConformations = conformations ?? conformationState.conformations;
    const resolvedActiveMode = activeMode ?? conformationState.activeMode;
    ...
    useImperativeHandle(ref, () => ({
      ...
      setConformations: (c, mode) => setConformationState({ conformations: c, activeMode: mode }),
      clearConformations: () => setConformationState({ conformations: null, activeMode: null }),
    }));
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className={`flex flex-row w-full border border-border rounded overflow-visible ${isZoomed ? 'invisible h-0 overflow-hidden' : ''}`}>
          ...
          <div className="flex flex-1 overflow-hidden" style={{ minWidth: 0 }}>
            <MolstarPanel
              ...
              representation={representation}
              conformations={resolvedConformations}
        ...
        {isZoomed && (
          ...
          >
            {/* Top bar */}
            <div
              className="flex-none flex items-center justify-between px-6 py-3 border-b border-border"
              style={{ backgroundColor: 'var(--color-bg-secondary, #111)' }}
            >
              <div className="flex items-center gap-3">
                ...
                <span className="font-mono text-xs text-text-muted">Monomer vs Homodimer</span>
              </div>
              <button
                type="button"
