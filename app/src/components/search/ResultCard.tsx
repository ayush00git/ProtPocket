...
export function ResultCard({ complex }: ResultCardProps) {
  ...
  const getDrugBadgeVariant = (count: number): 'unknown' | 'undrugged' | 'drugged' => {
    if (count === -1) return 'unknown';
    if (count === 0) return 'undrugged';
    return 'drugged';
  };

  const getDrugBadgeText = (count: number): string => {
