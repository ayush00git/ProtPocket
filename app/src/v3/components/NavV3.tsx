...
const HamburgerIcon = ({ open }: { open: boolean }) => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" strokeWidth="1.75" strokeLinecap="round" stroke="currentColor" aria-hidden="true">
    {/* Top bar: slides down + rotates into \ */}
    <line
    ...
    />
    {/* Bottom bar: slides up + rotates into / */}
    <line
  ...
  return (
    <>
      <div className="fixed top-4 left-0 right-0 z-50 flex justify-center items-center gap-2.5 pointer-events-none px-4">
        ...
        <button
          className="pointer-events-auto md:hidden inline-flex items-center justify-center w-10 h-10 rounded-full text-[#0B0F14]"
          style={pillBase}
          onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div
          className="fixed top-[72px] inset-x-4 z-50 md:hidden rounded-2xl overflow-hidden flex flex-col py-2"
          style={{
            ...pillBase,
            animation: 'navDropdown 220ms cubic-bezier(0.16,1,0.3,1) both',
          }}
        >
          <style>{`
          ...
          <Link
            ...
            className="px-5 py-3 text-[15px] text-[#0B0F14] no-underline hover:bg-[rgba(11,15,20,0.04)] transition-colors duration-150"
            style={{ fontWeight: 450 }}
          >
            Platform
          </Link>
